...
def main(args):
    ...
    if is_master(args, local=args.log_local):
        ...
        if (
            os.path.isdir(args.checkpoint_path)
            and not os.listdir(args.checkpoint_path)
            or "debug" in args.checkpoint_path.lower()
        ):
            print(f"Checkpoint path is empty. Deleting log path: {log_base_path}")
    ...
    # Setup wandb, checkpoint logging
    args.wandb = "wandb" in args.report_to or "all" in args.report_to
    args.checkpoint_path = os.path.join(log_base_path, "checkpoints")
    if is_master(args) and args.checkpoint_path:
    ...
    dist_model = None
    args.distill = (
        args.distill_model is not None or args.distill_pretrained is not None
    )
    if args.distill:
    ...
    dist_tokenizer = get_tokenizer(
        args.distill_model, cache_dir=args.cache_dir, context_length=args.force_context_length
    )
    preprocess_fn = (preprocess_train, preprocess_val, dist_preprocess_train, dist_preprocess_val) if args.distill else (preprocess_train, preprocess_val)
    data = get_data(args, preprocess_fn, epoch=start_epoch, tokenizer=tokenizer, dist_tokenizer=dist_tokenizer)
    ...
    if args.distill_loss == "clipkd":
        args.s_embed = get_model_config(args.model)["embed_dim"]
        args.t_embed = get_model_config(args.distill_model)["embed_dim"]

    loss = create_loss(args).to(model.device)
